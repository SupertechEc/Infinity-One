...
export class SubmenuComponent implements OnInit {
  ...
  getDataItem(): void {
    if (this.id !== 'new') {
      // this.loading = true;
      this.cf.getItemData('submenús', this.id).subscribe(data => {
        console.log(data.payload.data());
        this.imgUrl = data.payload.data().imagenUrl;
        this.f.setValue({
          nombre: data.payload.data().nombre,
          menuId: data.payload.data().menuId,
  ...
  close(): void {
    console.log('Salir de SUBMENÚS');
    this.router.navigate(['/dashboard/detalle-opciones'], { queryParams: { nombre: 'SUBMENÚS' } });
  }
  ...
  save(): void {
    ...
    if (this.f.valid) {
      ...
      if (this.id !== 'new') {
        ...
        this.cf.editItem('submenús', this.id, value).then(() => {
        ...
        this.cf.agregarItem(value, 'submenús').then(() => {
  ...
  uploadFile(event: any): void {
    ...
    const fileName = 'SUBMENU-' + this.f.get('nombre')?.value + '' + numram;
