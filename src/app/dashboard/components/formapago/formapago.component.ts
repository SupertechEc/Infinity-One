...
export class FormapagoComponent implements OnInit {
  ...
  getDataItem(): void {
    if (this.id !== 'new') {
      ...
      this.ia.getItemInfinity('formapago', parametros).subscribe(
        d => {
          ...
          this.setChange(d.retorno[0].activo);
        },
  ...
  close(): void {
    console.log('Salir de Forma de Pago');
    this.router.navigate(['/dashboard/detalle-opciones'], { queryParams: { nombre: 'FORMA PAGO' } });
  }
  ...
  save(): void {
    ...
    if (this.f.valid) {
      ...
      if (this.id !== 'new') {
        ...
        this.editItems(value, this.id, 'formapago', 'postgres');
      } else {
        // this.addItems('áreamercadeo', value, 'firebase');
        this.addItems('formapago', value, 'postgres');
      }
      console.log(value);
    }
  }
