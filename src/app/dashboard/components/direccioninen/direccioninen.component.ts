...
@Component({
  selector: 'app-direccioninen',
  templateUrl: './direccioninen.component.html',
  styleUrls: ['./direccioninen.component.css']
})
export class DireccioninenComponent implements OnInit {
  ...
  getDataItem(): void {
    ...
    if (this.id !== 'new') {
      ...
      this.ia.getItemInfinity('direccioninen', parametros).subscribe(
        d => {
  ...
  close(): void {
    ...
    this.router.navigate(['/dashboard/detalle-opciones'], { queryParams: { nombre: 'DIRECCIÓN INEN' } });
  }
  ...
  save(): void {
    ...
    if (this.f.valid) {
      ...
      this.registro = true;
      if (this.id !== 'new') {
        this.editItems(value, this.id, 'direccioninen', 'postgres');
      } else {
        this.addItems('direccioninen', value, 'postgres');
      }
      console.log(value);
    }
  }
